...
Vector Machine to achive excellent results of 93\% accuracy.
...
The following paragraphs desribe character, lexical and syncatic features 
...
using some sort of NLP tool, and a single text is characterized by the presence
...
used. Stamatatos et al.\ \cite{stamatatos2001computer} used noun, verb and
prepositional phrases frequencies. Using a Part--of--speech (POS) tagger one can obtain POS tags and
POS tags \emph{n}--grams frequencies by which excellent results can be achived
...
Croatian language. We use word length, sentence length, punctuation marks 
...
First we tested all the features separately and then we tested those features in
different combinations. The process of SVM parameter selection is very time
consuming so not all feature combinations were tested. Results of the evalutaion
...
itself. Therefore, for every class precision and recall are calculated and then
used to calculate total weighted $F_1$ measure. For particular class $c$,
precision is ratio of number of correctly classified documents in $c$ and number
of all documents which are classified as $c$. Recall is ratio of number of correctly
classified documents in $c$ and number of all documents in $c$. $F_1$ measure
