...
written by 25 different authors where the articles are not evenly distributed
across authors. The number of texts per author in the set is shown on Figure
\ref{fig:articlesPerAuthor}. The articles are not all the same size either
--- the lowest average number of words in the text per author is 315 words, and
...
impossible to conclude if these methods are generally effective
...
Next three features we used are syntax--based
features and thus some sort of NLP (Natural Language Processing) tool
is required. Croatian language is morphologically
complex and it is problematic to construct a robust NLP tool. The tool
we used, given in \cite{snajder08automatic},  
used cannot distinguish between different homographs. The tool is not perfect
because the method is feature vector is created by counting the appearances of different lexical categories. The used categories are adverbs, adpositions, conjunctions, particles, interjections, nouns, verbs, adjectives and pronouns. They are obtained by syntactic analysis of the Croatian language. Result is stored as a vector (nine--dimensional vector). Each component of the vector is divided by the number of words in the text.
...
Feature based on the frequency of vowel occurence (a, e, i, o, u) is
...
Features suggested here all have weak discriminatory power on their own
However, they proved very useful in the combination with other features, as
