...
branch} considers word \emph{n}--grams as features, i.e.\ features consisting of
sequences of consecutive words. Nevertheless, due to the feature explosion, it
tends to use only \emph{n}--grams up to three words.
\end{description}
...
own, such as adverbs, prepositions, conjunctions, or articles. They usually indicate
...
morphosyntactic descriptors are obtained by POS (Part--Of--Speech) tagging. Using
\emph{n}--grams as features can produce very large number of dimensions, therefore
only 3--grams are used.
...
U daljnjem radu potrebno je evaluirati metode temeljene na \emph{n}--gramima riječi i
