...
\begin{abstract}
In this work we investigate the use of various character, lexical, syntactical
level features and their combination for automatic authorship attribution. Since
the most text representation features, reported in the related work, are language
specific we examine their application on the texts written in Croatian language.
Data set considered is quite heterogeneous and consists of articles (journals)
from daily Croatian newspaper written by 25 different authors.
...
texts of rather small size (on average <1000 words). Although the comparison of
the results has proved to be difficult due to the different types of data sets
